...
int main(int argc, char **argv) {
 ...
 if (argp[0] && argp[1])
  usage(argv[0]);
 if (*argp && !freopen(*argp, "r", __stdinp))
  perror(*argp), exit(1);
...
int cpp(int argc, char **argv) {
 ...
 if (!(cmd = (char*) calloc(i + sizeof("/usr/bin/cpp"), sizeof(char))))
  return -1;

 ((__builtin_object_size (cmd, 0) != (size_t) -1) ? __builtin___strcpy_chk (cmd, "/usr/bin/cpp", __builtin_object_size (cmd, 2 > 1)) : __inline_strcpy_chk (cmd, "/usr/bin/cpp"));
 for (argp = argv; *++argp; )
  if (**argp == '-' && index("CDEIPU", (*argp)[1]))
   ((__builtin_object_size (cmd, 0) != (size_t) -1) ? __builtin___strcat_chk (cmd, " ", __builtin_object_size (cmd, 2 > 1)) : __inline_strcat_chk (cmd, " ")), ((__builtin_object_size (cmd, 0) != (size_t) -1) ? __builtin___strcat_chk (cmd, *argp, __builtin_object_size (cmd, 2 > 1)) : __inline_strcat_chk (cmd, *argp));
